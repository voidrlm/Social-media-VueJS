      ...
      <v-col cols="12" md="8" lg="6">
        <div class="d-flex align-center mt-5 mb-2 px-4">
          ...
          <span class="text-h5 font-weight-bold"
            >{{ getGreetingData }}, {{ $store.getters.currentUser.name }}!</span
          >
        </div>
    ...
    >
      <template v-slot:activator="{ on, attrs }">
        <v-fab-transition>
          <v-btn
            ...
            elevation="20"
            :class="
              $vuetify.theme.dark ? 'black--text white' : 'black--text accent'
            "
      ...
      <template v-slot:default>
        <v-card class="rounded-xl">
          <v-toolbar color="accent" elevation="0" class="text-h6"
            >Create a new post.</v-toolbar
          >
          <v-form v-model="validPost">
            <v-card-text class="pb-0 mb-n4">
              ...
              <v-text-field
                ...
                @input="isValidUrl"
                :rules="[
                  (v) => isValidUrl() || v === '' || 'Invalid image URL',
                ]"
              ></v-text-field>
            </v-card-text>
          </v-form>
